...
const EPaperViewer = () => {
    ...
    useEffect(() => {
        const fetchEPaper = async () => {
            ...
            try {
                ...
                // Note: Strapi date fields might need specific formatting. 
                // Using simple date matching for now.
                const dateString = selectedDate.toISOString().split('T')[0];
                
                const queryParams = new URLSearchParams({
                    'populate[pages][populate]': 'image',
                    'filters[publishedDate][$eq]': dateString,
                    'locale': locale
                });

                const data = await fetchAPI(`/e-papers?${queryParams}`);
                
                if (data?.data?.length > 0) {
                    const epaper = data.data[0].attributes || data.data[0];
                    const pageData = epaper.pages.map((p, i) => ({
                        id: p.id,
    ...
    const formatDateStr = (dateObj) => {
        return dateObj.toLocaleDateString(locale === 'bn' ? 'bn-BD' : 'en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
    };
                        ...
                        <div className="d-flex gap-2 position-relative">
                            ...
                            <div className="position-relative">
                                <div className="custom-dropdown-btn" onClick={() => { setIsMonthOpen(!isMonthOpen); setIsDayOpen(false); setIsYearOpen(false); }}>
                                    <span>{displayMonths[selectedDate.getMonth()]}</span>
                                    <Icons.ChevronDown />
                                </div>
                                {isMonthOpen && (
                                    <div className="custom-dropdown-menu">
                            ...
                            <button className="btn btn-primary d-flex align-items-center gap-2 px-3 rounded-3" onClick={() => {/* Trigger Fetch if needed (useEffect handles it) */}}>
                                <Icons.Search />
            ...
            <div className="container-fluid px-lg-5 py-4 flex-grow-1">
                {loading ? (
                    <div className="text-center py-5">
                       <Skeleton style={{ width: '100%', height: '600px', borderRadius: '12px' }} />
                    </div>
                ) : pages.length > 0 ? (
                                    ...
                                    <div className="d-flex flex-column gap-3">
                                        {pages.map((page, index) => (
                                            <div key={page.id} className={`page-thumbnail cursor-pointer bg-white p-2 shadow-sm ${currentPage === index ? 'active' : ''}`} onClick={() => handlePageChange(index)}>
                                                ...
                                                <ImageWithFallback src={page.image} alt={page.title} className="img-fluid rounded" style={{ height: 'auto', width: '100%' }} />
                            ...
                            <div className="viewer-card h-100 d-flex flex-column">
                                <div className="epaper-toolbar border-bottom p-3 d-flex justify-content-between align-items-center bg-light bg-opacity-50">
                                    <div className="d-flex align-items-center gap-3">
                                        <h4 className="epaper-current-title mb-0 fw-bold text-dark">{pages[currentPage].title}</h4>
                                        <span className="epaper-date text-muted border-start ps-3 small">{formatDateStr(selectedDate)}</span>
                                    </div>
                                ...
                                <div className="epaper-info-body card-body">
                                    <ul className="list-unstyled mb-0 d-flex flex-column gap-2 small">
                                        <li className="d-flex justify-content-between">
                                            <span className="epaper-info-label text-muted">Date:</span>
                                            <span className="epaper-info-value fw-bold">{selectedDate.toLocaleDateString()}</span>
                                        </li>
                                         <li className="d-flex justify-content-between">
                                            <span className="epaper-info-label text-muted">Pages:</span>
                                            <span className="epaper-info-value fw-bold">{pages.length}</span>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                    ...
                    <div className="text-center py-5 bg-white rounded shadow-sm">
                        <Icons.News />
                        <h4 className="mt-3">No E-Paper found for this date.</h4>
                    ...
                    <div className="modal-image-container animate__animated animate__zoomIn">
                        ...
                        />
                         <div className="text-center mt-3 text-white">
                            <h5 className="mb-0">{pages[currentPage].title}</h5>
                            <small className="opacity-75">{formatDateStr(selectedDate)}</small>
                        </div>
                    </div>
